...
default_source = 'https://github.com/kanghuawu/my-test-repo'
...
def doDelete(sshConnection, appName):
	print "> Stopping..."
	doFlaskStop(sshConnection)
...
def doFlaskStart(sshConnection, appName):
	print "> Start app..."
	commands = 'python '+ appName + "/app.py &\n"
	# print commands
...
def doFlaskStop(sshConnection):
	print "> Stop app..."
	commands = "kill -9 $(ps aux | grep python | awk '{print $2}')\n"
...
if __name__ == "__main__":
	...
	parser.add_argument("--action", help='Action with your repo on Raspberry Pi. Default value: ' + default_action, choices=['deploy', 'stop', 'delete'], default=default_action)
	...
	if sshConnection is not None:
		...
		if not isGitInstalled:
			...
			elif action == ACTION_STOP and isAppExist:
				doStop(sshConnection, appName)
			elif action == ACTION_DELETE and isAppExist:
				doDelete(sshConnection, appName)
			else:
				doNothing()
